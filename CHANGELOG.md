...
- プロバイダーごとのアイコンを足した。入力欄の上のプロバイダーチップ、プロバイダー選択のクイックメニュー、設定画面の「プロバイダ選択」の見出しに出る
  - 各社の公式ロゴは使っていない。Google・Anthropic・OpenAI・Groq のいずれも、第三者の製品でロゴを使うには事前の許可かライセンスが要るとブランドガイドラインに書いているため、ロゴ画像を同梱することも、似せた図形を自作することもしない
  - 代わりに MD3SDK に入っている Material Symbols のグリフ (Apache-2.0) を使い、「事業者の API を直接叩く」「ローカルの CLI を起動する」「この PC で動かす」「ブラウザごし」「手でコピーして貼り付ける」「外部のエージェントがつなぐ」といった、つなぎ方の違いで絵を選んだ。つなぎ方が同じプロバイダーは同じ絵になるので、その場合は色で見分ける
  - 配色も各社のブランド色には寄せていない。Google はブランドカラーの流用も禁じている。ライトとダークのどちらの地色でも読める明度の 2 色組を独自に組み、むしろブランド色とは意図的にずらした (Claude は青緑、Gemini は紅紫、DeepSeek は橙、Groq は緑)
  - チップとメニュー項目にカーソルを置くと、つなぎ方の説明が出る。同じ絵になるプロバイダーがあるので、絵だけでは何を表しているのか伝わりきらないため
  - モデルのチップとメニューには付けていない。プロバイダーを見分けるのが目的で、両方に付けると行が混むだけになる
  - 設定画面のドロップダウンの項目そのものには付けられなかった。MD3Dropdown は項目をコンストラクタの中で作って抱え込み、開いたときには別の要素の下へ移すので、MD3SDK を変えないかぎり外から触れない。見出しの左に、いま選んでいるプロバイダーのアイコンを出すだけにしてある
...
- 推論の強さの上限をモデル単位にした。従来はプロバイダー単位で、Claude API ならどのモデルでも xHigh / Max が選べた
  ...
  - Claude CLI でも `xhigh` と `max` を選べるようにした。Claude Code は Claude API と同じ 5 段階を受け付けるのに、3 段階しか出しておらず、上の 2 つを選ぶ手段が無かった。どの値まで通るかはモデルによって違うので、絞り込みは他と同じくモデルの集合との積で行う
  - `ultracode` は選択肢に入れていない。これは Claude Code 側の設定で、モデルには `xhigh` を送ったうえでワークフローの組み立てを追加するものなので、強さの段階として並べると他のプロバイダーと意味が揃わない
...
- 入力欄の上のチップ行で、モデル名のチップだけが幅を譲るようにした。アイコンの分だけ行が長くなるので、狭いウィンドウで右端の「ログを保存」が押し出されないようにするため。幅が足りないときは長いモデル名の末尾が … になる
- プロバイダー選択のクイックメニューの最大幅を 320px から 360px に広げた。アイコンを足すと、最も長い「Web Browser (Gemini / ChatGPT / Copilot)」が 1 行に収まらなくなる。項目の高さは 48px 固定なので、折り返すと 2 行目が切れて読めない
