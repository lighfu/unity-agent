...
(function () {
  ...
  const PROVIDERS = [
    ...
    { name: "Antigravity CLI (agy)", kind: "cli", auth: "CLI ログイン", ja: "Gemini CLI の後継。agy のヘッドレスモードで会話を実行。", en: "Successor to Gemini CLI. Runs the conversation through agy's headless mode.", "zh-TW": "Gemini CLI 的後繼。透過 agy 的無頭模式執行對話。", zh: "Gemini CLI 的后继。通过 agy 的无头模式执行对话。" },
    { name: "Gemini CLI (legacy)", kind: "cli", auth: "CLI ログイン", ja: "Gemini CLI を介した会話実行。個人アカウントでの提供は終了し、API キーや法人向けの利用者のみ。", en: "Conversation through Gemini CLI. No longer offered to personal accounts; API-key and enterprise users only.", "zh-TW": "透過 Gemini CLI 執行對話。已停止提供給個人帳戶，僅限 API 金鑰與企業使用者。", zh: "通过 Gemini CLI 执行对话。已停止向个人账户提供，仅限 API 密钥与企业用户。" },
